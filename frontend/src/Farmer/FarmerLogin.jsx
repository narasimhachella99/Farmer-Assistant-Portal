...
const FarmerLogin = () => {
  ...
  const handleSubmit = async () => {
    try{
      const res = await axios.post(`http://localhost:8080/api/v1/farmerLogin`, data);
        ...
        <div className='col-start-3 col-end-6 mt-5'>
          ...
          <div className="  bg-gray-100 rounded-lg p-8  bg-gradient-to-l from-sky-600  to-green-600  md:ml-auto w-96 mt-10 md:mt-0">
            ...
            <p class="text-sm font-light text-gray-500 dark:text-gray-400 mt-1">
              SignUp for new account? <a href="/farmerregister" class="font-medium text-white hover:underline dark:text-primary-500">Register here</a>
            </p>
          </div>
        </div>
      </div>
